...
$start = idx($vars, 's', 0);
$field_to_update = 'supporting_actors';


$i = $start;
do {
  // Make sure you start with the right films you want to update
  $film = get_object_from_sql(
    sprintf("select  * from films where wiki_handle is not null and "
	    ."supporting_actors is null "
	    ."limit %d, %d",
	    $i++, 1));
  if (!$film) {
  ...
  if ($new_fields_from_wiki && $field_to_update) {
    $temp = array();
    $temp[$field_to_update] = idx($new_fields_from_wiki, $field_to_update) ?
      $new_fields_from_wiki[$field_to_update] : null;
    $new_fields_from_wiki = $temp;
  }
  $new_fields_from_wiki = array_filter($new_fields_from_wiki);
...
function parse_wiki_text($film) {
  ...
  if (!idx($film, 'supporting_actors')) {
    foreach ($dom->find('h2') as $h2) {
      if (stripos($h2->plaintext, 'cast') !== false ) {
        $node = $h2->next_sibling();
	$r['supporting_actors'] = process_people($node, 'supporting_actor');
      }
    }
  }
  ...
  if (!idx($film, 'wiki_plot')) {
    $wiki_plot = null;
    foreach ($dom->find('h2') as $h2) {
      if (stripos($h2->plaintext, 'plot') !== false
	  || stripos($h2->plaintext, 'synopsis') !== false) {
	$wiki_plot = '';
	$node = $h2->next_sibling();
	while ($node && stripos($node->outertext,
				'class="mw-headline"') === false) {
...
function process_people($dom, $type) {
  hlog('processing people of type '.$type);
  $people = array();
  foreach ($dom->find('a') as $a) {
    $handle = rem($a->href, array('/wiki/', '/w/index.php?title=', '&amp;action=edit&amp;redlink=1'));
