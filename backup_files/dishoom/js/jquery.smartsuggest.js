									...
									if (options.showEmptyCategories || (!options.showEmptyCategories && group['data'].length != 0))
										...
										$.each(group['data'], function (j, item) {
											if (count < limit)
												...
												output += '<table class="search-table" border="0" cellspacing="0" cellpadding="0" width="100%"><tr>';
												output += (item['image'] != undefined && options.showImages) ? '<td width="50"><img width="40" height="40" src="'+item['image']+'" /></td>'+"\n" : '';
												output += '<td>';
												output += '<p>';
