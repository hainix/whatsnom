...
jQuery.fcbkListSelection = function(elem, width, height, row){
    ...
    var addToSelected = function(obj){
        if (obj.hasClass("itemselected")) {
            ...
            $("#num_seen").text(parseInt($("#num_seen").text(), 10) - 2);
            $("#num_seen2").text(parseInt($("#num_seen2").text(), 10) - 2);
    ...
    var createTabs = function(elem, width){
        var html = '<div id="multiselect_list_filters" style="width:' + (parseInt(width, 10) + 2) + 'px;">' +
        '<ul class="selections" id="selections"><li id="view_all" class="view_on">' +
        '<a onclick="return false;" href="#">All Movies</a></li><li id="view_selected" class="">' +
        '<a onclick="return false;" href="#">Seen (<strong id="view_selected_count">0</strong>)</a></li>' +
        '<li id="view_unselected" class=""><a onclick="return false;" href="#">Haven\'t Seen</a></li></ul>' +
